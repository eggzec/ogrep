...
// These are only honored while inside a w:r (run) element — w:tab can
// also appear inside w:pPr/w:tabs as a tab-STOP definition (unrelated to
// actual inserted text), so tracking "are we inside a run" is what keeps
// that case from being misread as inserted text.
//
// Blank filtering: body paragraphs are emitted even when empty, mirroring
// the plain-text plugin's behavior of emitting every line so paragraph
// numbering stays predictable and dense. Table cells, headers, footers,
// footnotes, and comments are only emitted when they contain non-blank
// text, since those are "extra" units (not part of the primary
// sequential addressing) and emitting blank ones (e.g. empty
// alignment-only paragraphs common in headers/footers) would just be
// noise.
//
